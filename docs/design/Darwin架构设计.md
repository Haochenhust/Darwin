...
单进程、无 Docker、无 K8s，所有依赖都在宿主 Mac mini 上直接运行。选型偏向稳定、TypeScript 原生支持好、社区活跃的库。
...
| 层次                | 技术                             | 版本     | 选择理由                                                                                                                                                        |
| ----------------- | ------------------------------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **语言运行时**         | Node.js                        | 22 LTS | 与飞书 SDK、Claude Agent SDK 官方支持的运行时一致；LTS 保障稳定性                                                                                                               |
| **开发语言**          | TypeScript                     | ^5.7   | 强类型约束降低 vibe coding 时的隐式 bug；四层架构的接口契约靠类型系统保障                                                                                                               |
| **Agent Runtime** | @anthropic-ai/claude-agent-sdk | ^2.1   | 官方 SDK，内置 MessageStream、session resume、MCP 集成、PreCompact Hook；是整个系统的智能核心                                                                                    |
| **MCP 工具框架**      | @modelcontextprotocol/sdk      | ^1.12  | 配合 createSdkMcpServer() 实现进程内 MCP Server，无需独立子进程；Skills-first 哲学的基础设施                                                                                       |
| **飞书通信**          | @larksuiteoapi/node-sdk        | ^1.x   | 官方 Node SDK，内置 WebSocket 长连接（免公网 IP）、断线重连、API 鉴权；覆盖消息收发和飞书 API（文档/日历/知识库）                                                                                   |
| **数据库**           | better-sqlite3                 | ^11.0  | 同步 API 与 Node.js 单线程模型完美匹配，无 async 回调复杂度；WAL 模式保证崩溃安全；零外部依赖，Mac mini 直接运行                                                                                   |
| **输入校验**          | zod                            | ^4.0   | MCP Tool 的参数 schema 定义，运行时类型校验；与 TypeScript 类型系统深度集成，schema 即类型                                                                                             |
| **定时调度**          | cron-parser                    | ^5.0   | 轻量级 cron 表达式解析，支持 cron / interval / once 三种任务类型；不引入 bull/redis 等重型队列                                                                                        |
| **日志系统**          | pino + pino-pretty             | ^9.0   | 结构化 JSON 日志，性能极佳；pino-pretty 在开发时提供人类可读格式；子 logger 携带 chatId/groupName 上下文                                                                                  |
| **测试框架**          | vitest                         | ^3.0   | 与 TypeScript + ESM 原生兼容，无需额外配置；API 与 Jest 兼容，学习成本低；Vitest 对你来说主要用于测**纯逻辑函数**（不依赖网络/Claude API 的部分），比如队列、session 状态机、消息解析。涉及飞书和 Claude 的部分，你在飞书里直接发消息验证效果更快。 |
| **进程守护**          | launchd（macOS 原生）              | -      | Mac mini 原生进程管理，KeepAlive=true 保证崩溃自动重启；开机自启无需第三方工具（pm2/supervisor）                                                                                         |
| **包管理**           | npm                            | -      | Node.js 标准工具链，package-lock.json 锁定依赖版本                                                                                                                      |


### 关键依赖说明

**为什么不用 Docker？**
单用户个人助手场景，Docker 带来的隔离收益远小于其增加的运维复杂度（镜像管理、volume 挂载、网络配置）。无 Docker 意味着 Darwin 可以直接调用 `osascript`、`pbcopy` 等 macOS 原生命令，能力更强、延迟更低。

**为什么不用 Redis / BullMQ？**
GroupQueue 是单进程内存队列，无需持久化（消息已落 SQLite）；并发量极低（个人使用，同时处理的 Group 数量 < 10）。引入 Redis 只会增加部署复杂度，没有收益。

**为什么 better-sqlite3 而不是 Prisma / TypeORM？**
ORM 的抽象层在 vibe coding 场景下反而是障碍——Claude Code 直接生成带 Repository 模式的 SQL 函数比维护 ORM schema 迁移更高效。better-sqlite3 的同步 API 在 Node 单线程里无竞争风险。
...
{
  "dependencies": {
    "@anthropic-ai/claude-agent-sdk": "^2.1.0",
...
- NanoClaw: [https://github.com/Haochenhust/ClawPartner/tree/feat/chenhao](https://github.com/Haochenhust/ClawPartner/tree/feat/chenhao);
- GraceBot: [https://github.com/Haochenhust/GraceBot](https://github.com/Haochenhust/GraceBot);
- OpenClaw: [https://github.com/openclaw/openclaw](https://github.com/openclaw/openclaw);
- agentara: [https://github.com/MagicCube/agentara](https://github.com/MagicCube/agentara);
